...
type Item = { pk: string; name: string; itemCode?: string; barcode?: string; purchasePrice?: number };
...
export default function NewPurchaseReturnPage() {
  ...
  function updateLine(id: string, field: "qty" | "purchasePrice" | "discountPct", value: number) {
    setLines((prev) =>
      prev.map((l) => {
        if (l.id !== id) return l;
        const updated = { ...l, [field]: value };
        const discountAmt = (updated.purchasePrice * updated.discountPct) / 100;
        updated.unitCost = updated.purchasePrice - discountAmt;
  ...
  const discountAmt = (subtotal * discountOnAll) / 100;
  ...
  async function handleCreate() {
    ...
    setSaving(true);
    setMsg(null);
    const res = await api("/api/purchases/returns", {
      method: "POST",
      body: JSON.stringify({
    ...
    setSaving(false);
    if (!res.ok) { setMsg({ type: "err", text: res.error || "Failed to create purchase return." }); return; }
    router.push("/purchase/returns/list");
  }

  return (
    <PageScaffold title="Purchase Return" subtitle="Create New Purchase Return">
      <div className="bg-white border border-slate-200 rounded-sm p-5 mb-4">
          ...
          <table className="w-full text-sm border-collapse">
            <thead>
              <tr className="bg-blue-700 text-white text-xs">
                {["Item Name", "Quantity", "Purchase Price(LKR)", "Discount(%)", "Unit Cost(LKR)", "Total Amount(LKR)", "Action"].map((h) => (
        ...
        <div className="mb-5">
          ...
          <table className="w-full text-sm border border-t-0 border-slate-200">
            ...
            <tbody>
              <tr>
                <td colSpan={6} className="px-4 py-6 text-center text-slate-400 text-sm">Payments Pending!!</td>
              </tr>
            </tbody>
          </table>
        </div>
        ...
        <div className="grid grid-cols-2 gap-3">
          ...
          <button
            type="button"
            onClick={() => router.push("/purchase/returns/list")}
