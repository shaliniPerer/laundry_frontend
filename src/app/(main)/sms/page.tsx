...
type Tab = "log" | "customers" | "events" | "templates";
...
type Customer = {
  ...
  phone?: string; dob?: string; status?: string;
};
...
function isBirthdayToday(dob?: string): boolean {
  if (!dob) return false;
  return (dob.length > 5 ? dob.slice(5, 10) : dob) === todayMMDD();
}
...
export default function SmsPage() {
  ...
  const TABS: { id: Tab; label: string }[] = [
    ...
    { id: "customers", label: "Customer Sync" },
      ...
      <div className="flex gap-1 bg-slate-100 rounded-xl p-1 w-fit mb-6 flex-wrap">
        {TABS.map((t) => (
          <button key={t.id} onClick={() => setTab(t.id)}
            className={`px-4 py-1.5 rounded-lg text-sm font-medium transition-colors ${tab === t.id ? "bg-white text-teal-700 shadow-sm" : "text-slate-500 hover:text-slate-700"}`}>
            {t.label}
          </button>
        ))}
      </div>
        ...
        <div className="space-y-4">
          <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
            {[
              ...
              { label: "Reachable (phone)", value: customers.filter((c) => c.phone).length,  color: "text-teal-600",   icon: <Phone className="w-4 h-4" /> },
                  ...
                  <tbody className="divide-y divide-slate-100">
                    {customers.map((c) => (
                      <tr key={c.pk} className="hover:bg-slate-50">
                        ...
                        <td className="px-5 py-3 font-mono text-slate-600 text-xs">{c.phone ?? "—"}</td>
                        <td className="px-5 py-3 text-slate-600 text-xs">{c.dob ?? <span className="text-slate-300">not set</span>}</td>
            ...
            {filteredTemplates.map((tpl) => {
              ...
              return (
                <div key={tpl.id} className="bg-white rounded-2xl border border-slate-200 p-5 card-shadow flex flex-col gap-3">
                  ...
                  <div className="flex items-start justify-between gap-2">
                    <div className="min-w-0">
                      <div className="flex items-center gap-2 flex-wrap">
                        ...
                        {tpl.isCustom && (
                          <span className="px-1.5 py-0.5 rounded text-xs font-medium bg-violet-50 text-violet-700 border border-violet-200 shrink-0">custom</span>
                        )}
                        {tpl.isCustomized && !tpl.isCustom && (
                          <span className="px-1.5 py-0.5 rounded text-xs font-medium bg-amber-50 text-amber-700 border border-amber-200 shrink-0">edited</span>
                        )}
                      </div>
                  ...
                  {/* Description (edit mode, custom only) */}
                  {isEditing && tpl.isCustom && (
