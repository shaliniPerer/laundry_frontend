...
const COLUMNS = [
  ...
  { name: "GST Number", required: false },
  { name: "TAX Number", required: false },
...
export default function ImportSuppliersPage() {
  ...
  function downloadExample() {
    ...
    const example = "ABC Supplies,0771234567,abc@example.com,0771234567,GST001,TAX001,Sri Lanka,Western,Colombo,00100,123 Main St,500";
    const blob = new Blob([[header, example].join("\n")], { type: "text/csv" });
    const url = URL.createObjectURL(blob);
    const a = document.createElement("a");
    a.href = url; a.download = "suppliers_example.csv"; a.click();
  ...
  async function handleImport(e: React.FormEvent) {
    ...
    const res = await api<{ imported: number }>("/api/suppliers/import", {
      method: "POST",
      body: JSON.stringify({ rows }),
    });
    setImporting(false);
    if (!res.ok) { setMsg({ type: "err", text: res.error || "Import failed." }); return; }
    setMsg({ type: "ok", text: `Successfully imported ${res.data?.imported ?? 0} suppliers.` });
    if (fileRef.current) fileRef.current.value = "";
    setFileName("");
  }

  return (
    <PageScaffold title="Import Suppliers" subtitle="Add/Update Supplier">
      {/* Upload form */}
        ...
        <form onSubmit={handleImport}>
          <div className="flex items-start gap-4 mb-6">
            <label className="text-sm text-slate-600 shrink-0 pt-1.5 w-36 text-right">
              Import Suppliers<span className="text-red-500">*</span>
            </label>
