...
export default function NewItemCategoryPage() {
  ...
  async function submit(e: React.FormEvent) {
    ...
    setMsg(null);
    setSaving(true);
    const res = await api("/api/items/categories", { method: "POST", body: JSON.stringify({ name, kind: "item", description }) });
    setSaving(false);
  ...
  return (
    <PageScaffold title="Category" subtitle="Add/Update Category">
      <div className="bg-white border border-slate-200 rounded-sm p-6">
        <p className="text-sm font-semibold text-slate-700 mb-5">Please Enter Valid Data</p>
        <form onSubmit={submit} className="max-w-lg space-y-4">
          <div className="flex items-center">
            <label className="w-36 text-right pr-4 text-sm text-slate-600 shrink-0">Service Name<span className="text-red-500">*</span></label>
          ...
          <div className="flex justify-center gap-4 pt-2">
            ...
            <button type="button" onClick={() => router.push("/items/categories/list")} className="w-44 py-2.5 bg-amber-400 hover:bg-amber-500 text-white font-bold rounded text-sm transition-colors">Close</button>
          </div>
        </form>
      </div>
    </PageScaffold>
